...
int main(){
	...
	int choice,key;
	while(1){
		printf("Enter 1 to insert node, 2 to search for a node, 3 to get inorder predecessor of a node, 4 to get inorder successor of a node, 5 to delete a node, 6 to print inorder traversal, 7 to exit\n");
		scanf("%d",&choice);
		switch(choice){
			case 1: 
			...
			case 2:
				...
				if(temp1->left!=NULL)printf("Left child: %d\n",temp1->left->val);
				else printf("Left root null\n");
				if(temp1->right!=NULL)printf("Right child: %d\n",temp1->right->val);
